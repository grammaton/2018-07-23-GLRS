...
dbat_group(x) = hgroup("Double Bass Amplification", x);
...
inputsel = in_group(hgroup("[0] INPUTS", _,_,_,_,_,_,_,_,_,_,_,_ : !,!,!,!,!,!,!,!,_,_,_,! : *(in_gain), *(in_gain), *(in_gain) : hmeter,hmeter,hmeter ));
...
fl_out = fl_sixch
  with{
    at1 = de.sdelay(192000,1024,ac.mt2samp(nentry("distance ch1", 2., 0., 1000., 0.1))) ;
    at2 = de.sdelay(192000,1024,ac.mt2samp(nentry("distance ch2", 2., 0., 1000., 0.1))) ;
    at3 = de.sdelay(192000,1024,ac.mt2samp(nentry("distance ch3", 4., 0., 1000., 0.1))) ;
    at4 = de.sdelay(192000,1024,ac.mt2samp(nentry("distance ch4", 4., 0., 1000., 0.1))) ;
    at5 = de.sdelay(192000,1024,ac.mt2samp(nentry("distance ch5", 6., 0., 1000., 0.1))) ;
    at6 = de.sdelay(192000,1024,ac.mt2samp(nentry("distance ch6", 1., 0., 1000., 0.1))) ;

    fl_sixch = flute_group(vgroup("[2] OUTPUTS", _ <:
    hgroup("[0]", at1,at2,at3,at4,at5,at6) :
    fi.highpass(2,666),fi.highpass(2,666),fi.highpass(2,666),fi.highpass(2,666),fi.highpass(2,666), fi.lowpass(2,120) :
    _,_,_,_,_, *(in_gain):
    hmeter, hmeter, hmeter, hmeter, hmeter, hmeter));
};
